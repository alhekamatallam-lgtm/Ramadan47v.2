...
import { Search, Image as ImageIcon, X, ArrowRight, Grid, List } from 'lucide-react';
...
const PhotoGallery: React.FC<PhotoGalleryProps> = ({ photos, mosques, onBack }) => {
  const [selectedMosqueCode, setSelectedMosqueCode] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState('');
  const [lightboxImage, setLightboxImage] = useState<string | null>(null);

  // Group photos by mosque_code (from tags)
  const mosquePhotoCounts = useMemo(() => {
    const counts: Record<string, number> = {};
    photos.forEach(photo => {
      if (photo.tags) {
        counts[photo.tags] = (counts[photo.tags] || 0) + 1;
      }
    });
    return counts;
  }, [photos]);
  ...
  const filteredMosques = useMemo(() => {
    return mosques
      ...
      .map(m => ({
        ...m,
        photoCount: mosquePhotoCounts[m.mosque_code] || 0
      }))
      .sort((a, b) => b.photoCount - a.photoCount);
  }, [mosques, searchQuery, mosquePhotoCounts]);
    ...
    <div className="space-y-8 animate-in fade-in" dir="rtl">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-3xl font-black text-[#003366]">
              {selectedMosqueCode ? selectedMosqueName : 'معرض الصور الميداني'}
            </h2>
            <p className="text-slate-400 text-sm font-bold">
              {selectedMosqueCode 
                ? `توثيق بصري لجميع الأنشطة في الموقع (${mosquePhotos.length} صورة)` 
                : 'تصفح التوثيق البصري لجميع المواقع والمساجد'}
            </p>
          </div>
        </div>
      ...
      {!selectedMosqueCode ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredMosques.map(mosque => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl transition-colors ${mosque.photoCount > 0 ? 'bg-[#0054A6]/10 text-[#0054A6]' : 'bg-slate-50 text-slate-300'}`}>
              ...
              <div className="flex flex-col items-center">
                <span className="text-2xl font-black text-[#003366] tabular-nums">{mosque.photoCount}</span>
                <span className="text-[10px] text-slate-400 font-black uppercase">صورة</span>
              </div>
            </button>
          ))}
        </div>
          ...
          {mosquePhotos.length > 0 ? (
            mosquePhotos.map((photo, idx) => (
              <div 
                ...
                onClick={() => setLightboxImage(photo.secure_url)}
              >
                <img 
      ...
      {lightboxImage && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10">
          ...
          <div className="relative max-w-full max-h-full animate-in zoom-in duration-300">
            <img 
              src={lightboxImage} 
              alt="Full size" 
